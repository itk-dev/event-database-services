# Changelog

![keep a changelog](https://img.shields.io/badge/Keep%20a%20Changelog-v1.1.0-brightgreen.svg?logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%)

All notable changes to this project will be documented in this file.

See [keep a changelog] for information about writing changes to this log.

## [Unreleased]

## [1.1.0] - 2026-07-10

- [PR-2](https://github.com/itk-dev/event-database-services/pull/2)
  Upgrade Elasticsearch to 8.19.18
- Fix a broken link in the README

## [1.0.1] - 2025-03-12

## [1.0.0] - 2024-12-13

[keep a changelog]: https://keepachangelog.com/en/1.1.0/
[Unreleased]: https://github.com/itk-dev/event-database-services/compare/1.1.0...HEAD
[1.1.0]: https://github.com/itk-dev/event-database-services/compare/1.0.1...1.1.0
[1.0.1]: https://github.com/itk-dev/event-database-services/releases/tag/1.0.1
[1.0.0]: https://github.com/itk-dev/event-database-services/releases/tag/1.0.0
